...
fileWW = TFile("~/3DayLifetime/out_WW_all_PUPPI.root", "READ")
fileZZ = TFile("~/3DayLifetime/out_ZZ_all_PUPPI.root", "READ")
fileHH = TFile("~/3DayLifetime/out_HH_all_PUPPI.root", "READ")
fileTT = TFile("~/3DayLifetime/out_TT_all_PUPPI.root", "READ")
fileJJ = TFile("~/3DayLifetime/out_QCD_all_PUPPI.root", "READ")
fileBB = TFile("~/3DayLifetime/out_BB_all_PUPPI.root", "READ")
treeWW = fileWW.Get("run/jetTree")
treeZZ = fileZZ.Get("run/jetTree")
treeHH = fileHH.Get("run/jetTree")
treeTT = fileTT.Get("run/jetTree")
treeJJ = fileJJ.Get("jetTree")
treeBB = fileBB.Get("run/jetTree")
...
print "number of WW events: ", len(arrayWW)
print "number of ZZ events: ", len(arrayZZ)
print "number of HH events: ", len(arrayHH)
print "number of JJ events: ", len(arrayJJ)
print "number of BB events: ", len(arrayBB)
print "number of TT events: ", len(arrayTT)
...
newArrayBB = []
numPerSample = 80000
for entry in arrayWW[:numPerSample]:
    a = list(entry)
    newArrayWW.append(a)
for entry in arrayZZ[:numPerSample]:
    a = list(entry)
    newArrayZZ.append(a)
for entry in arrayHH[:numPerSample]:
    a = list(entry)
    newArrayHH.append(a)
for entry in arrayTT[:numPerSample]:
    a = list(entry)
    newArrayTT.append(a)
for entry in arrayJJ[:numPerSample]:
    a = list(entry)
    newArrayJJ.append(a)
for entry in arrayBB[:numPerSample]:
    a = list(entry)
    newArrayBB.append(a)
...
cm = metrics.confusion_matrix(mlp.predict(trainData[:]), targetData[:])
plt.figure()
targetNames = ['j', 'W', 'Z', 'H', 't', 'b']
plot_confusion_matrix(cm.T, targetNames, normalize=True)
plt.savefig('confusion_PUPPI.pdf')
plt.show()
...
print mlp.score(trainData[:], targetData[:])
...
joblib.dump(mlp, 'BEST_mlp_6bin_PUPPI.pkl') 
joblib.dump(scaler, 'BEST_scaler_6bin_PUPPI.pkl')
